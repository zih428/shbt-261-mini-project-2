...
The project also has clear limitations. VOC 2007 is small, so architecture ranking may be more sensitive to the inductive biases of the benchmark than to true large-scale robustness. More importantly, the pretrained initializations differ across families, so the headline comparison should be read as a pragmatic benchmark of end-to-end systems rather than as a clean architecture-only study. The official validation split is treated correctly as a test set, but there is still only one split, and the internal development split is only 31 images, missing bicycle entirely and representing several classes only once. The hardware environment also shapes what is practical: larger transformer variants or full SAM2 fine-tuning were deliberately excluded because they would not have been a responsible use of time on the available device. Finally, although the report now includes bootstrap confidence intervals on official-validation mIoU, it does not yet include repeated-seed retraining, alternate-split reruns, or an original-resolution boundary study. The bootstrap analysis quantifies test-set variability, but it does not remove the model-selection noise induced by the tiny internal development split.

\section{Conclusion}

This study benchmarks four representative segmentation systems and seven ablations on Pascal VOC 2007 under a common, validation-driven training pipeline. SegFormer-B2 is the strongest model by the primary overlap metrics, DeepLabV3+ is the strongest CNN baseline and the best HD95 model, and SAM2 becomes competitive only after lightweight LoRA adaptation. The ablation suite shows that encoder capacity, augmentation strength, and training budget all matter materially, while the default cross-entropy-plus-Dice loss remains a sound compromise. The added bootstrap analysis also shows that the locked-test ranking is not being determined by only a handful of validation images, even though split-level training noise remains a real limitation. More broadly, the project demonstrates that even a course-scale benchmark can support a meaningful empirical paper if the evaluation protocol is locked, the figures are interpreted carefully, and the conclusions are stated at the level the evidence actually supports.
