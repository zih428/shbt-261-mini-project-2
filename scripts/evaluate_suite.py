#!/usr/bin/env python3
"""Evaluate multiple trained checkpoints and save analysis artifacts for each."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--configs", nargs="+", required=True, help="Experiment configs to evaluate.")
    ...
    parser.add_argument("--checkpoint-name", default="checkpoint_best.pth", help="Checkpoint filename inside each run dir.")
...
def evaluate_single_config(
    *,
    ...
    checkpoint_path = artifact_root / "runs" / experiment_name / checkpoint_name
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"Missing checkpoint for {experiment_name}: {checkpoint_path}")

    dataset_bundle = build_dataset_bundle(config["data"], data_root=data_root, metadata_dir=config["data"]["metadata_dir"])
    eval_dataset = dataset_bundle[split]
    eval_batch_size = batch_size_override or int(config["data"].get("eval_batch_size", config["data"]["batch_size"]))
    ...
    eval_dir = ensure_dir(artifact_root / "evals" / f"{experiment_name}_{split}")
    ...
    train_metrics_path = checkpoint_path.parent / "metrics.json"
    ...
    print(f"Evaluation complete for {experiment_name}. Artifacts saved to {eval_dir}")
